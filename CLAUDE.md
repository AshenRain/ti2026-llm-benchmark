...
- **2026-08-10** — context pack frozen, prompt finalised, odds snapshot taken (14:00 CEST)
- **2026-08-11** — all model runs collected
- **2026-08-12** — coherence analysis, post #1 published, repo pushed
...
- **The odds snapshot is taken in sync with the model runs, not later.** `odds.csv` is frozen at `2026-08-10T14:00+02:00`, alongside the context-pack freeze and immediately before the runs were collected, so the market baselines forecast from the same information the models had. Refreshing the snapshot closer to the tournament would hand the market entrants days of extra news — roster confirmations, scrim reports, late line movement — that the models never saw, and the comparison would stop being a fair one. The snapshot date does not move, even if the lines do.
...
- **No canonical team-name list was ever put in the prompt.** `prompt.md` says to "use the exact team names listed in the briefing," but `context_pack.md` itself presents three teams under two textual forms — a compact section header (`BoomBoys / BetBoom Team`, `Team Vision / PARIVISION`, `HULIGANI (ex-L1GA TEAM)`) and, separately, an explicit "X plays as Y" sentence — without ever saying which one is "the" name. This was discovered when GPT returned the section-header form verbatim for all three, while every other model normalised to the short form on its own initiative. Resolving it is a gap in our spec, not a model error: `score.py`'s `TEAM_SPEC_AMBIGUITY` table maps exactly these three verbatim strings to their canonical names, and doing so does **not** downgrade `parse_level` or count as a repair (contrast `TEAM_SYNONYMS`, for shorthand nothing in the prompt asked for, which does). Every occurrence is logged and surfaced in the coherence report for disclosure in the public README — this is a design limitation being disclosed, not swept under the rug.
...
odds.csv           bookmaker snapshot, 2026-08-10T14:00+02:00
results.csv        actual outcomes, filled after 08-23
score.py           scoring and charts
charts/            PNGs generated by `py score.py charts`, e.g. column_sums.png
requirements.txt   matplotlib -- the one non-stdlib dependency, needed only for `charts`
CLAUDE.md          this file
